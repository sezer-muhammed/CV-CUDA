...
def benchmark_script(
    ...
    nsys_root_path = "/opt/nvidia/nsight-systems/2025.5.1/"
    ...
    if not os.path.isfile(nsys_binary_path):
        raise ValueError(
            "Unable to locate nsys binary at %s. Make sure you have nsight-systems installed."
            % nsys_binary_path
        )
    ...
    cpu_json_path = os.path.join(output_dir, "perf_report_nvtx_pushpop_trace.json")
    gpu_json_path = os.path.join(output_dir, "perf_report_nvtx_gpu_proj_trace.json")

    # Debug: Check JSON file sizes and existence
    logger = logging.getLogger("benchmark.py")
    cpu_json_size = (
        os.path.getsize(cpu_json_path) if os.path.exists(cpu_json_path) else 0
    )
    gpu_json_size = (
        os.path.getsize(gpu_json_path) if os.path.exists(gpu_json_path) else 0
    )
    logger.info(
        f"NVTX JSON files - CPU: {cpu_json_size} bytes, GPU: {gpu_json_size} bytes"
    )

    cpu_range_info = parse_nvtx_pushpop_trace_json(cpu_json_path)
    gpu_range_info = parse_nvtx_gpu_proj_trace_json(gpu_json_path)

    # Debug: Count ranges parsed
    cpu_range_count = (
        sum(
            len(threads)
            for process in cpu_range_info.values()
            for threads in process.values()
        )
        if cpu_range_info
        else 0
    )
    gpu_range_count = (
        sum(
            len(threads)
            for process in gpu_range_info.values()
            for threads in process.values()
        )
        if gpu_range_info
        else 0
    )
    logger.info(f"Parsed NVTX ranges - CPU: {cpu_range_count}, GPU: {gpu_range_count}")

    if cpu_range_count == 0 and gpu_range_count == 0:
        logger.warning(
            "WARNING: No NVTX ranges captured! This may indicate:\n"
            "  1. NVTX Python package incompatibility with Python version\n"
            "  2. nsys unable to hook into the Python process\n"
            "  3. NVTX markers not being emitted properly"
        )
    # Process
    ...
    # Debug: Show what keys the benchmark dict has
    benchmark_keys = list(benchmark_dict["data"].keys())
    logger.info(
        f"Benchmark dict has {len(benchmark_keys)} keys. Sample (first 5): {benchmark_keys[:5]}"
    )
    logger.info(
        f"NVTX ranges has {len(mean_ranges_info)} keys. Sample (first 5): {list(mean_ranges_info.keys())[:5]}"
    )
    logger.info(f"obj_name prepend: '{benchmark_dict['meta']['obj_name']}'")
    ...
    for k in mean_ranges_info:
        ...
        # Remove all ':' from NVTX keys (NSYS uses ':' as hierarchy markers)
        clean_key = k.replace(":", "")
        prepended_key = os.path.join(benchmark_dict["meta"]["obj_name"], clean_key)
    ...
    logger.info(
        f"Key matching: {matched_keys} matched, "
        f"{len(unmatched_keys)} unmatched out of {len(mean_ranges_info)} total"
    )
    if len(unmatched_keys) > 0 and len(unmatched_keys) <= 5:
        logger.info(f"Sample unmatched keys: {unmatched_keys[:5]}")
    elif len(unmatched_keys) > 5:
        logger.info(f"Sample unmatched keys (first 5): {unmatched_keys[:5]}")
