    ...
    def generate_decoded_frames(self):
        for packet in self.nvDemux:
            for decodedFrame in self.nvDec.Decode(packet):
                cvcudaTensor = cvcuda.as_tensor(
                    cvcuda.as_image(decodedFrame.nvcv_image(), cvcuda.Format.U8)
                )
                if cvcudaTensor.layout == "NCHW":
                    ...
                    yield cvcuda.reformat(cvcudaTensor, "NHWC")
                else:
...
class VideoBatchEncoder:
    ...
    def __call__(self, batch):
        ...
        cvcuda_tensor = cvcuda.as_tensor(batch.data, cvcuda.TensorLayout.NCHW)
...
class ImageBatchDecoder:
    ...
    def __call__(self):
        ...
        # Convert the decoded images to cvcuda tensors in a list.
