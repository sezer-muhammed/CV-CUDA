...
def test_wrap_host_buffer_arg_keywords():
    img = cvcuda.Image(buffer=np.ndarray([5, 7], np.float32), format=cvcuda.Format.F32)
    assert img.size == (7, 5)
    assert img.format == cvcuda.Format.F32

    img = cvcuda.as_image(
        buffer=util.to_cuda_buffer(np.ndarray([5, 7], np.float32)),
        format=cvcuda.Format.F32,
    )
    assert img.size == (7, 5)
    assert img.format == cvcuda.Format.F32
...
def test_wrap_host_buffer_infer_format_arg_keywords():
    img = cvcuda.Image(buffer=np.ndarray([5, 7], np.float32))
    assert img.size == (7, 5)
    assert img.format == cvcuda.Format.F32

    img = cvcuda.as_image(buffer=util.to_cuda_buffer(np.ndarray([5, 7], np.float32)))
    assert img.size == (7, 5)
    assert img.format == cvcuda.Format.F32
...
def test_image_zeros():
    img = cvcuda.Image.zeros((67, 34), cvcuda.Format.F32)
